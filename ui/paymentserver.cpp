...
const int AVH_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString AVH_IPC_PREFIX("bitcoin:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(AVH_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(AVH_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(AVH_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(AVH_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
